...
* New values of the fields must satisfy the validation rules as outlined for the `add` command.
...
This command clears all entries from the BloodNet system.
...
data from your previous BloodNet home folder.<br>
<br>
...
2. If you try to `delete` a donor and that donor has donation records, those donation records will have to be deleted
   first using the `deletedonation` command before the donor can be fully deleted.
